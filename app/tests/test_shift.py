...
def test_create_shift(client: TestClient, auth_headers):
    r = client.post("/shifts/", json=SHIFT_DATA, headers=auth_headers)
...
def test_get_all_shifts_filter_by_day(client: TestClient, auth_headers):
    ...
    r = client.get("/shifts/?day_of_week=monday", headers=auth_headers)
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_update_shift(client: TestClient, auth_headers):
    client.post("/shifts/", json=SHIFT_DATA, headers=auth_headers)
    r = client.patch("/shifts/1", json={"instructor": "Maria"}, headers=auth_headers)
    assert r.status_code == 200
...
def test_update_shift_not_found(client: TestClient, auth_headers):
    r = client.patch("/shifts/999", json={"instructor": "Maria"}, headers=auth_headers)
    assert r.status_code == 404


def test_delete_shift(client: TestClient, auth_headers):
    client.post("/shifts/", json=SHIFT_DATA, headers=auth_headers)
    r = client.delete("/shifts/1", headers=auth_headers)
    assert r.status_code == 200
    assert client.get("/shifts/1", headers=auth_headers).status_code == 404


def test_delete_shift_not_found(client: TestClient, auth_headers):
    r = client.delete("/shifts/999", headers=auth_headers)
    assert r.status_code == 404
...
def test_get_shift_availability_not_found(client: TestClient, auth_headers):
    r = client.get("/shifts/999/availability?date=2026-06-02", headers=auth_headers)
    assert r.status_code == 404